...
const EmailNotificationsList = () => {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      const response = await api.get('/api/email-template', { params });
  ...
  const handleRefresh = async () => {
    ...
    toast.success('Email templates refreshed successfully.');
  };
  ...
  const handleToggleStatus = async (id, e) => {
    e.stopPropagation();
    try {
      const response = await api.post(`/api/email-template/toggle/${id}`);
      if (response.data && response.data.success) {
    ...
    },
    {
      ...
      render: (item) => (
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/configurations/email-notifications/edit/${item.id}`);
          }}
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
          <button
            type="button"
            title={item.status === 'active' ? 'Inactivate email template' : 'Activate email template'}
          ...
          <button
            ...
            onClick={() => navigate(`/configurations/email-notifications/edit/${item.id}`)}
            className="w-10 h-10 flex items-center justify-center text-neutral-600 dark:text-neutral-400 bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 hover:border-brand-300 dark:hover:border-brand-700 hover:text-brand-600 dark:hover:text-brand-400 rounded-lg transition-all cursor-pointer outline-none"
          >
  ...
  // Adjust column sizing to mimic exactly the user mockup
  const gridTemplateColumns = "minmax(150px, 1fr) minmax(220px, 1.2fr) minmax(160px, 1fr) minmax(120px, 1fr) minmax(120px, 170px)";
  ...
  return (
    <div className="flex flex-col w-full select-none">
      <div className="mb-3 shrink-0">
        <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-3 min-w-0">
          <div className="min-w-0 flex-1">
            <ModuleHeader title="Email Notifications" breadcrumb="Configurations / Email Notifications" />
          </div>
      ...
      <div className="flex flex-col w-full gap-4">
        ...
        <InfoNote>
          <strong>Note:</strong> Predefined templates are used by the system to dispatch automated notifications. Toggling status disables sending specific emails. Click on template name or edit button to edit subject and content.
        </InfoNote>
      </div>
    </div>
  );
