...
const PushNotificationEdit = () => {
  ...
  useEffect(() => {
    const fetchTemplateDetail = async () => {
      setLoading(true);
      try {
        const response = await api.get(`/api/push-notification/detail/${id}`);
        if (response.data && response.data.success) {
          ...
          toast.error('Push notification template not found.');
          navigate('/configurations/push-notifications');
        }
      } catch (error) {
        console.error(error);
        toast.error('Failed to load push notification details.');
        navigate('/configurations/push-notifications');
      } finally {
        setLoading(false);
      }
  ...
  const handleSave = async (e) => {
    ...
    setIsSaving(true);
    try {
      const response = await api.post('/api/push-notification/edit', {
        id,
        templateName,
        body,
        status,
      ...
      if (response.data && response.data.success) {
        toast.success(response.data.message || 'Push notification updated successfully.');
        navigate('/configurations/push-notifications');
      }
    } catch (error) {
      console.error(error);
      const msg = error.response?.data?.message || 'Failed to update push notification.';
      toast.error(msg);
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <form onSubmit={handleSave} className="flex flex-col w-full min-w-0 select-none pb-8">
      ...
      <div className="flex items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400 font-medium mb-1">
        ...
        <button 
          type="button" 
          onClick={() => navigate('/configurations/push-notifications')}
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-neutral-900 dark:text-white">Edit Push Notification</h1>
          <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
            Modify automated mobile push alert templates
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
        ...
        <div className="flex-1 w-full flex flex-col gap-6 bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-xl p-6 shadow-sm">
          ...
          <div className="flex flex-col gap-1.5 font-normal col-span-2">
