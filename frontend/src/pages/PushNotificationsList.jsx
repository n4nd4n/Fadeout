...
const PushNotificationsList = () => {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      const response = await api.get('/api/push-notification', { params });
  ...
  const handleRefresh = async () => {
    ...
    toast.success('Push notifications refreshed successfully.');
  };
  ...
  const handleToggleStatus = async (id, e) => {
    e.stopPropagation();
    try {
      const response = await api.post(`/api/push-notification/toggle/${id}`);
      if (response.data && response.data.success) {
  ...
  const columns = [
    {
      ...
      sortable: true,
      render: (item) => (
        <span className="text-sm text-neutral-700 dark:text-neutral-300 font-mono text-xs truncate" title={item.templateId}>
          {item.templateId}
        </span>
      ),
    },
    {
      ...
      render: (item) => (
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/configurations/push-notifications/edit/${item.id}`);
          }}
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
          <button
            type="button"
            title={item.status === 'active' ? 'Inactivate push template' : 'Activate push template'}
          ...
          <button
            ...
            onClick={() => navigate(`/configurations/push-notifications/edit/${item.id}`)}
            className="w-10 h-10 flex items-center justify-center text-neutral-600 dark:text-neutral-400 bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 hover:border-brand-300 dark:hover:border-brand-700 hover:text-brand-600 dark:hover:text-brand-400 rounded-lg transition-all cursor-pointer outline-none"
          >
  ...
  const gridTemplateColumns = "minmax(220px, 1.8fr) minmax(220px, 1.5fr) minmax(140px, 1fr) minmax(110px, 0.8fr) minmax(120px, 120px)";
  ...
  return (
    <div className="flex flex-col w-full select-none">
      <div className="mb-3 shrink-0">
        <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-3 min-w-0">
          <div className="min-w-0 flex-1">
            <ModuleHeader title="Push Notifications" breadcrumb="Configurations / Push Notifications" />
          </div>
      ...
      <div className="flex flex-col w-full gap-4">
        ...
        <InfoNote>
          <strong>Note:</strong> Predefined templates are used by the system to dispatch automated mobile push alerts. Toggling status disables sending specific push alerts. Click on template name or edit button to edit name and content.
        </InfoNote>
      </div>
    </div>
  );
