...
const EmailNotificationEdit = () => {
  ...
  useEffect(() => {
    const fetchTemplateDetail = async () => {
      setLoading(true);
      try {
        const response = await api.get(`/api/email-template/detail/${id}`);
        if (response.data && response.data.success) {
          ...
          navigate('/configurations/email-notifications');
        }
      } catch (error) {
        console.error(error);
        toast.error('Failed to load email template details.');
        navigate('/configurations/email-notifications');
      } finally {
        setLoading(false);
      }
  ...
  const insertVariable = (variable) => {
    // Insert template variable at cursor position in contenteditable
    if (editorRef.current) {
  ...
  const handleSave = async (e) => {
    ...
    setIsSaving(true);
    try {
      const response = await api.post('/api/email-template/edit', {
        id,
        templateName,
        subject,
        body,
      ...
      console.error(error);
      const msg = error.response?.data?.message || 'Failed to update email template.';
      toast.error(msg);
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <form onSubmit={handleSave} className="flex flex-col w-full min-w-0 select-none pb-8">
      ...
      <div className="flex items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400 font-medium mb-1">
        ...
        <button 
          type="button" 
          onClick={() => navigate('/configurations/email-notifications')}
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-neutral-900 dark:text-white">Edit Email Template</h1>
          <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
            Modify automated notification system templates
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                required
                className="w-full h-11 px-3.5 text-sm bg-white dark:bg-neutral-950 text-neutral-900 dark:text-white border border-neutral-200 dark:border-neutral-800 rounded-lg focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all"
              />
            </div>
          </div>
            ...
            <div className="flex flex-col w-full border border-neutral-200 dark:border-neutral-800 rounded-lg overflow-hidden bg-neutral-50 dark:bg-neutral-900/50">
              ...
              <div className="flex flex-wrap items-center gap-1 p-2 bg-neutral-100 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-800">
                    ...
                    <div className="absolute left-0 mt-1 w-36 bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded shadow-lg z-50 py-1">
                      {formatOptions.map((opt) => (
                        <button
                          ...
                          className="w-full px-3 py-1.5 text-left text-xs text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-900 transition-colors"
                        >
                          {opt.label}
                        </button>
                      ))}
                ...
                <button
                  ...
                  className={cn(
                    "w-8 h-8 flex items-center justify-center rounded text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-800 transition-colors",
                ...
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center rounded text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-800 transition-colors"
                >
                  <Quote size={16} />
                </button>
                    ...
                    <div className="absolute left-0 mt-1 w-32 bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded shadow-lg z-50 py-1">
                      {alignOptions.map((opt) => (
                        <button
                          ...
                          className="w-full px-3 py-1.5 flex items-center text-left text-xs text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-900 transition-colors"
                        >
                          {opt.icon}
                          {opt.label}
                        </button>
